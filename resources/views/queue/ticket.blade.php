...
@section('content')
{{-- Auto-refresh setiap 10 detik untuk update status (termasuk saat dipanggil petugas) --}}
@if(in_array($queue->status, ['approved', 'waiting', 'called']) && empty($isExpiredTicket))
...
<div class="py-12">
    <div class="max-w-md mx-auto bg-white shadow-lg rounded-2xl overflow-hidden">
        @php
            $queueLabel = $queue->queue_number ?: 'Menunggu Scan';
        @endphp
        ...
        <div class="p-6">
            ...
            <div class="text-center mb-6">
                ...
                <p class="text-5xl font-black text-green-700 mt-1">{{ $queueLabel }}</p>
                @if(!$queue->queue_number && $queue->status === 'approved')
                    <p class="text-xs font-semibold uppercase tracking-wide text-blue-600 mt-2">Nomor diterbitkan setelah QR discan di loket</p>
                @endif
            ...
            @if($queue->status === 'approved' && ($queue->qr_path || $queue->token) && empty($isExpiredTicket))
                <div class="text-center mb-6">
                    <p class="text-gray-500 text-sm mb-3">Tunjukkan QR Code ini di loket pendaftaran untuk mendapatkan nomor antrian FIFO</p>
                ...
                <div class="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6">
                    ...
                    <ol class="mt-2 text-sm text-amber-700 list-decimal list-inside space-y-1">
                        ...
                        <li>Petugas akan scan QR dan menerbitkan nomor antrian Anda</li>
            ...
            @elseif($queue->status === 'served' && !empty($needsPharmacyStep) && !empty($nextPharmacyQueue))
                <div class="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6">
                    ...
                    <a href="{{ route('queue.show', $nextPharmacyQueue->id) }}" class="mt-3 inline-flex items-center px-4 py-2 rounded-lg text-sm font-semibold text-white" style="background: linear-gradient(135deg, #f59e0b, #d97706);">
                        Lihat Tiket Farmasi ({{ $nextPharmacyQueue->queue_number ?: 'Menunggu nomor' }})
                    </a>
                </div>
