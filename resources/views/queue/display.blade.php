...
<body class="min-h-screen text-white"
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            ...
            <div class="bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl p-4 shadow-2xl">
                ...
                <div class="relative w-full rounded-xl overflow-hidden bg-black/50" style="padding-bottom: 56.25%;">
                    <video id="local-video-player" data-local-video-urls='@json($localVideoUrls)' class="absolute inset-0 w-full h-full {{ count($localVideoUrls) > 0 ? '' : 'hidden' }}" autoplay muted playsinline preload="auto" controls>
    ...
    <script>
        ...
        let localVideoUrls = [];
        if (localVideoPlayerEl && localVideoPlayerEl.dataset.localVideoUrls) {
            try {
                localVideoUrls = JSON.parse(localVideoPlayerEl.dataset.localVideoUrls);
            } catch (e) {
                console.warn('Failed to parse local video URLs payload:', e);
                localVideoUrls = [];
            }
        }
