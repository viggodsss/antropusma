...
<div style="padding: 32px 0;">
    <div style="max-width: 1280px; margin: 0 auto; padding: 0 24px;">
        @php
            ...
            $dashboardBundleSections = ['dashboard', 'scan-qr', 'verifikasi-pasien', 'persetujuan-antrian', 'menunggu-scan', 'ringkasan-antrian'];
            $showDashboard = in_array($activeSection, $dashboardBundleSections, true);
            $showScanQr = in_array($activeSection, $dashboardBundleSections, true);
            $showWaAudit = $activeSection === 'wa-audit';
            $showStatistik = $activeSection === 'statistik-ringkas';
            $showKlaster = $activeSection === 'klaster-keluhan';
            $showAkun = $activeSection === 'akun-pasien';
            $showVerifikasi = in_array($activeSection, $dashboardBundleSections, true);
        ...
        @if($showWaAudit)
        @php
            $waSentCount = $waNotificationLogs->where('status', 'sent')->count();
            $waFailedCount = $waNotificationLogs->where('status', 'failed')->count();
            $waSkippedCount = $waNotificationLogs->where('status', 'skipped')->count();
        @endphp
        <div id="wa-audit" class="admin-card" style="margin-bottom: 28px;">
            <div class="admin-card-header" style="justify-content: space-between; flex-wrap: wrap; gap: 12px;">
                <div>
                    <h2 style="font-size: 16px; font-weight: 700; color: #111827;">Audit Notifikasi WhatsApp</h2>
                    <p style="font-size: 13px; color: #6b7280; margin-top: 2px;">Monitor status kirim WA (scan, dipanggil, selesai) dengan retry & fallback.</p>
                </div>
                <div style="display: flex; gap: 8px; flex-wrap: wrap;">
                    <span class="admin-badge badge-success">Sent: {{ $waSentCount }}</span>
                    <span class="admin-badge badge-danger">Failed: {{ $waFailedCount }}</span>
                    <span class="admin-badge badge-gray">Skipped: {{ $waSkippedCount }}</span>
                </div>
            </div>

            @if($waNotificationLogs->count() > 0)
                <div style="max-height: 420px; overflow: auto;">
                    <table class="admin-table">
                        ...
                        <tbody>
                            @foreach($waNotificationLogs as $log)
                                <tr>
                                    <td>{{ optional($log->created_at)->format('d-m H:i:s') }}</td>
                                    <td>
                                        @if($log->queue)
                                            <span style="font-weight: 700; color: #2563eb;">{{ $log->queue->queue_number ?? '-' }}</span>
                                            <div style="font-size: 11px; color: #6b7280;">{{ $log->queue->patient_name }}</div>
                                        @else
                                            -
                                        @endif
                                    </td>
                                    <td><span class="admin-badge badge-info">{{ strtoupper($log->event) }}</span></td>
                                    <td>{{ $log->provider_name }}</td>
                                    <td>{{ $log->attempt }}</td>
                                    <td>
                                        @if($log->status === 'sent')
                                            <span class="admin-badge badge-success">sent</span>
                                        @elseif($log->status === 'failed')
                                            <span class="admin-badge badge-danger">failed</span>
                                        @else
                                            <span class="admin-badge badge-gray">skipped</span>
                                        @endif
                                    </td>
                                    <td>{{ $log->http_status ?? '-' }}</td>
                                    <td>{{ \Illuminate\Support\Str::limit((string) ($log->error_message ?? '-'), 90) }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
            ...
            @else
                <div class="admin-empty-state">
                    <p style="color: #9ca3af; font-size: 13px;">Belum ada log notifikasi WhatsApp.</p>
                </div>
            @endif
        </div>
        @endif
                            ...
                            @foreach($todayComplaints as $item)
                                <div class="complaint-card">
                                    <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 6px;">
                                        <span class="admin-badge badge-info">{{ $item->queue_number }}</span>
                    ...
                    <table class="admin-table">
                        ...
                        <tbody>
                            @foreach($approvedQueues as $queue)
                                <tr>
                                    <td><span style="font-weight: 700; color: #0891b2;">{{ $queue->queue_number ?? '-' }}</span></td>
                                    <td style="font-weight: 500;">{{ $queue->patient_name }}</td>
                                    <td>
                                        @if($queue->has_bpjs)
                                            <span class="admin-badge badge-success">Ya</span>
                        ...
                        <tbody>
                            @foreach($staleApprovedQueues as $queue)
                                <tr>
                                    ...
                                    <td><span style="font-weight: 700; color: #dc2626;">{{ $queue->queue_number ?? '-' }}</span></td>
                                    <td style="font-weight: 500;">{{ $queue->patient_name }}</td>
                                    <td>{{ $queue->service_type }}</td>
                                    <td>
...
<script>
    ...
    const scanFromImageFile = async (file) => {
        ...
        if (typeof html5QrcodeCtor === 'undefined') {
        ...
        try {
            const imageScanner = new html5QrcodeCtor(qrReaderId);
    ...
    const startScanner = async () => {
        ...
        try {
            html5Qrcode = new html5QrcodeCtor(qrReaderId);
