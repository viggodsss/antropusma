...
class QueueController extends Controller
{
    ...
    // Valid service types dengan prefix
    ...
    {
        $klasters = array_keys(self::SERVICE_TYPES);
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'service_type' => 'required|string|in:' . implode(',', array_keys(self::SERVICE_TYPES)),
        ...
        $queue = Queue::create([
            'user_id' => Auth::id(),
            // Nomor antrian ditetapkan saat QR discan di loket (FIFO berdasarkan scan).
            'queue_number' => null,
            'patient_name' => $request->patient_name,
            'nik' => $request->nik,
            'no_hp' => $request->filled('no_hp') ? preg_replace('/[^0-9+]/', '', $request->no_hp) : null,
            'complaint' => $request->complaint,
            'service_type' => $request->service_type,
            'queue_date' => now()->toDateString(),
            'status' => 'approved', // Tiket terbit, menunggu scan di Puskesmas
            'token' => $token,
    ...
    public function show(Queue $queue)
    {
        ...
        // Render QR dari storage sebagai data URI agar tidak tergantung symlink /storage.
        if ($queue->status === 'approved' && !empty($queue->qr_path) && Storage::disk('public')->exists($queue->qr_path)) {
            try {
                $bytes = Storage::disk('public')->get($queue->qr_path);
                if ($bytes !== '') {
                    $mime = 'image/png';
                    ...
                    $qrImageDataUri = 'data:' . $mime . ';base64,' . base64_encode($bytes);
                }
            } catch (\Throwable $e) {
                Log::warning('Gagal memuat QR dari storage: ' . $e->getMessage());
            }
        }

        return view('queue.ticket', compact(
            'queue',
            'nextPharmacyQueue',
            'needsPharmacyStep',
            'isFinalAtPharmacy',
    ...
    // Panggil antrian berikutnya (FIFO berdasarkan created_at)
