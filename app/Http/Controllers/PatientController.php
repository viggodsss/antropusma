...
class PatientController extends Controller
{
    ...
    /**
     * Display antrian ringkas untuk pasien.
     */
    public function queueDisplay(): View
    {
        $today = now()->toDateString();
        ...
        $calledQueues = Queue::query()
            ...
            ->where(function ($query) {
                $query->whereIn('called_by_role', ['admin', 'petugas'])
                    ->orWhereNull('called_by_role');
            })
            ->whereDate('queue_date', $today)
            ->orderByDesc('called_at')
            ->orderByDesc('id')
            ->get(['id', 'queue_number', 'service_type', 'called_at']);
        ...
        $waitingQueues = Queue::query()
            ...
            ->orderBy('id')
            ->get(['id', 'queue_number', 'service_type']);

        $calledByCluster = $calledQueues->groupBy(fn (Queue $queue) => $queue->clusterNumber() ?? 0);
        $waitingByCluster = $waitingQueues->groupBy(fn (Queue $queue) => $queue->clusterNumber() ?? 0);

        $clusterNames = [
            1 => 'Klaster 1 Manajemen',
            2 => 'Klaster 2 Ibu dan Anak',
            3 => 'Klaster 3 Usia Dewasa dan Lansia',
            4 => 'Klaster 4 Penyakit Menular & Kesling',
            5 => 'Klaster 5 Lintas Klaster',
        ];

        $clusters = collect(Queue::CLUSTER_SERVICE_MAP)
            ->map(function (array $services, int $clusterNumber) use ($calledByCluster, $waitingByCluster, $clusterNames): array {
                $calledQueue = $this->latestCalledQueueForCluster($calledByCluster->get($clusterNumber, collect()));
                $waitingNumbers = collect($waitingByCluster->get($clusterNumber, collect()))
                    ->pluck('queue_number')
                    ->filter()
                    ->values();
                ...
                return [
                    ...
                    'name' => $clusterNames[$clusterNumber] ?? ('Klaster ' . $clusterNumber),
                    'called_queue' => $calledQueue?->queue_number,
                    'waiting_numbers' => $waitingNumbers,
                    'waiting_count' => $waitingNumbers->count(),
                    'active_count' => $waitingNumbers->count() + ($calledQueue ? 1 : 0),
                ];
            })
            ->values();

        $currentCalled = $calledQueues->first();

        return view('patient.display', [
            'clusters' => $clusters,
            'currentCalledQueue' => $currentCalled?->queue_number,
            'currentCalledCluster' => $currentCalled?->clusterNumber(),
            'totalWaitingQueues' => $waitingQueues->count(),
            'totalActiveClusters' => $clusters->where('active_count', '>', 0)->count(),
            'lastUpdatedAt' => now(),
        ]);
    }

    private function latestCalledQueueForCluster(Collection $calledQueues): ?Queue
    {
        $latest = $calledQueues->sortByDesc(function (Queue $queue) {
            return $queue->called_at?->getTimestamp() ?? 0;
        })->first();

        return $latest instanceof Queue ? $latest : null;
    }

    /**
     * JSON endpoint: data display antrian per klaster (untuk real-time fetch tanpa reload).
     */
    public function queueDisplayData(): JsonResponse
    {
        $today = now()->toDateString();
        ...
        $calledByCluster  = $calledQueues->groupBy(fn (Queue $q) => $q->clusterNumber() ?? 0);
        $waitingByCluster = $waitingQueues->groupBy(fn (Queue $q) => $q->clusterNumber() ?? 0);
        ...
        $clusters = collect(Queue::CLUSTER_SERVICE_MAP)
            ->map(function (array $services, int $clusterNumber) use ($calledByCluster, $waitingByCluster, $clusterNames): array {
                ...
                return [
                    ...
                    'called_queue'    => $calledQueue?->queue_number,
                    'waiting_numbers' => $waitingNumbers,
                    'waiting_count'   => count($waitingNumbers),
                    'active_count'    => count($waitingNumbers) + ($calledQueue ? 1 : 0),
                ];
            })
            ->values();
        ...
        return response()->json([
            'current_called_queue'   => $currentCalled?->queue_number,
            'current_called_cluster' => $currentCalled?->clusterNumber(),
            'total_waiting_queues'   => $waitingQueues->count(),
            'total_active_clusters'  => $clusters->where('active_count', '>', 0)->count(),
            'clusters'               => $clusters,
